...
def create_environment(date=cfg.ENV_DATE_TOMORROW, lat=cfg.ENV_LAT_FAR_OUT, lon=cfg.ENV_LON_FAR_OUT):
    ...
    env.elevation = cfg.ENV_ELEVATION_FAR_OUT
...
def create_tanks(
    ...
    # 3. Oxidizer mass uncertainty
    total_ox_mass = np.random.uniform(
        total_ox_mass - cfg.OX_MASS_UNC,
        total_ox_mass
    )

    # Set OX and FUEL initial mass
    ...
    total_fuel_mass = vol_fuel * fuel_l.density
    gas_init_mass_fuel = cfg.GAS_INITIAL_MASS_FUEL

    total_fuel_mass = np.random.uniform(
        total_fuel_mass - cfg.FUEL_MASS_UNC,
        total_fuel_mass
    )

    liq_init_mass_fuel = total_fuel_mass
